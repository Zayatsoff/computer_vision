...
# Edited by https://github.com/Zayatsoff
...
parser.add_argument('--imgdir', default='images/source/*.JPG', type=str,
                    help='choose which face detector to use: MTCNN, FaceBoxes, Retinaface')
...
def load_model():
    if args.backbone=='MobileNet':
        ...
        checkpoint = torch.load('models\pytorch_face_landmark-master_edited\checkpoint\mobilefacenet_model_best.pth.tar', map_location=map_location)      
...
if __name__ == '__main__':
    ...
    model = load_model()
    model = model.eval()
    filenames=glob.glob(args.imgdir)
